      ...
      <div v-show="!sidebarOpen" class="app-card flex flex-col  overflow-hidden max-h-[calc(100vh - 40px)]">
        ...
        <header :class="{ '!bg-gray-100': currentSlide == APP_SETTING_PANEL }" class="modal-card-head b-line">
          ...
          <div class="is-flex is-align-items-center">
            <b-button v-if="currentSlide == APP_STORE_PANEL" :label="$t('Add a Containerized Application')" class="mr-2"
            ...
            <b-tooltip v-if="showExportButton" :label="$t('Export ComposeFile')" position="is-bottom" type="is-dark">
        ...
        <section v-if="currentSlide == APP_STORE_PANEL" :class="{ _hideOverflow: !isCasa }"
          ...
          <template v-if="!isLoadError">
            ...
            <!-- App list Start-->
            <AppStoreContent :filteredPageList="filteredPageList" :installedList="installedList"
              :currentInstallId="currentInstallId" :arch="arch"></AppStoreContent>
            <!-- App list End-->
          </template>
          <template v-else>
        ...
        <AppInstallLoadingFooter v-if="currentSlide == APP_INSTALLING_PANEL"
          :currentInstallAppError="currentInstallAppError" :cancelButtonText="cancelButtonText" @close="$emit('close')"
          @back="prevStep" />

        <AppSettingPanelFooter v-if="currentSlide == APP_SETTING_PANEL" :state="state" :isCasa="isCasa"
          :isLoading="isLoading" @install="checkComposeAppAndInstallComposeApp()" @update="updateApp()"
...
export default {
  ...
  computed: {
    ...
    transitionName() {
      return !this.sidebarOpen ? "slide-next-half" : "slide-next-half";
    },
  },
  watch: {
